...
static bool tcp_time_to_recover(struct sock *sk, int flag)
{
	...
	/* Trick#2: SACKs hint to losses. At least one packet
	 * is lost, so that we have holes in sequence space.
	 * The first condition is check against reordering, causing
	 * false fast retransmits.
	 */
	if (tp->sacked_out >= tp->reordering)
		return true;

	/* Trick#3: Pure duplicate ACKs. This is Reno variant of recovery.
	 * The first condition is check against reordering.
	 */
	if (tcp_is_reno(tp) && tcp_dupack_heuristics(tp) > tp->reordering)
		return true;
